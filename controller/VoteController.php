...
if ($action === 'vote' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($existing) {
            ...
            $user_vote = $type_vote;
            $pointsAjoutes = CivismeModel::ajouterPoints((int) $num_cin, 20, 'vote_crowdfunding', 100, 30);
        }
        ...
        ob_clean();
        echo json_encode(['success' => true, 'counts' => $counts, 'points_ajoutes' => $pointsAjoutes]);
    } catch (PDOException $e) {
        ob_clean();
