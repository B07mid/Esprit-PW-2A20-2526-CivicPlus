...
class TransportNetwork
{
    ...
    public function saveLine(?int $id, string $name, string $type): array
    {
        ...
        if (strlen($name) < 2 || strlen($name) > 80) {
            throw new InvalidArgumentException('Le nom de la ligne doit contenir entre 2 et 80 caracteres.');
        }

        if ($type !== '' && strlen($type) > 40) {
            throw new InvalidArgumentException('Le type de transport ne doit pas depasser 40 caracteres.');
        }
    ...
    public function saveStation(?int $id, string $name, $latitude, $longitude): array
    {
        ...
        if ($latitude < -90 || $latitude > 90) {
            throw new InvalidArgumentException('La latitude doit etre comprise entre -90 et 90.');
        }

        if ($longitude < -180 || $longitude > 180) {
            throw new InvalidArgumentException('La longitude doit etre comprise entre -180 et 180.');
        }
