document.addEventListener("DOMContentLoaded", function() {
    ...
    if(formAjout) {
        ...
        const dureeInput = document.getElementById('duree_validite_mois');
        const submitBtn = formAjout.querySelector('button[type="submit"]');

        function checkFormValidity() {
            let isValid = true;

            if (libelleInput.value.trim().length < 3) {
                libelleInput.classList.add("is-invalid"); libelleInput.classList.remove("is-valid");
                isValid = false;
            } else {
                libelleInput.classList.remove("is-invalid"); libelleInput.classList.add("is-valid");
            }

            if (dureeInput.value !== "" && parseInt(dureeInput.value) <= 0) {
                dureeInput.classList.add("is-invalid"); dureeInput.classList.remove("is-valid");
                isValid = false;
            } else {
                dureeInput.classList.remove("is-invalid"); 
                if(dureeInput.value !== "") dureeInput.classList.add("is-valid");
            }
        ...
        [libelleInput, dureeInput].forEach(el => {
