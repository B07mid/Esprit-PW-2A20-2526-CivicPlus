...
class Donation {
    ...
    private static $hasMethodePaiementColumn = null;
    ...
    private static function hasMethodePaiementColumn($pdo) {
        if (self::$hasMethodePaiementColumn !== null) {
            return self::$hasMethodePaiementColumn;
        }

        try {
            $stmt = $pdo->query("SHOW COLUMNS FROM donation LIKE 'methode_paiement'");
            self::$hasMethodePaiementColumn = (bool) $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            self::$hasMethodePaiementColumn = false;
        }

        return self::$hasMethodePaiementColumn;
    }

    private static function ensureMethodePaiementColumn($pdo) {
        if (self::hasMethodePaiementColumn($pdo)) {
            return;
        }

        try {
            $pdo->exec("ALTER TABLE donation ADD COLUMN methode_paiement VARCHAR(30) NOT NULL DEFAULT 'carte_bancaire' AFTER montant");
            self::$hasMethodePaiementColumn = true;
        } catch (PDOException $e) {
            self::$hasMethodePaiementColumn = false;
        }
    }
    ...
    public function ajouterDonation($pdo) {
        try {
            self::ensureMethodePaiementColumn($pdo);
            $hasMethodColumn = self::hasMethodePaiementColumn($pdo);
            $sql = $hasMethodColumn
                ? "INSERT INTO donation
                    (num_cin, id_projet, montant, methode_paiement, reference_transaction, statut_paiement)
                    VALUES (:num_cin, :id_projet, :montant, :methode_paiement, :reference_transaction, :statut_paiement)"
                : "INSERT INTO donation
    ...
    public static function getAllDonations($pdo) {
        try {
            self::ensureMethodePaiementColumn($pdo);
            $select = self::hasMethodePaiementColumn($pdo) ? '*' : "*, '' AS methode_paiement";
