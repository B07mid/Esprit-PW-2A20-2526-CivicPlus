...
document.addEventListener("DOMContentLoaded", function () {
    console.log("Backoffice Script v5.1 loaded - Table Alignment Fix");
    chargerSignalements();
});
        ...
        .then(text => {
            ...
            try {
                ...
                signalements.forEach(sig => {
                    ...
                    let styleLigne = '';
                    let indicatorSOS = '';
                    let badgePriorite = `<span class="badge bg-light text-dark border">${priorite}</span>`;

                    if (priorite === 'SOS_VITALE') {
                        styleLigne = 'background-color: #fff5f5;';
                        indicatorSOS = '<div style="position:absolute; left:0; top:0; bottom:0; width:4px; background:#ef4444;"></div>';
                        badgePriorite = `<span class="badge bg-danger shadow-sm"><i class="bi bi-exclamation-triangle-fill"></i> SOS</span>`;
                    } else if (priorite === 'Urgente') {
                        badgePriorite = `<span class="badge bg-warning text-dark">Urgente</span>`;
                    }

                    // Contenu de la colonne Avis
                    let avisContent = '<span class="text-muted small">Aucun</span>';
                    if (sig.note_citoyen) {
                        avisContent = `
                            <div class="cursor-pointer" onclick="ouvrirAvisCitoyen(${sig.id_signalement})" title="Voir l'avis" style="cursor:pointer;">
                                <span class="text-warning">${'★'.repeat(sig.note_citoyen)}${'☆'.repeat(5-sig.note_citoyen)}</span>
                            </div>
                        `;
                    }
                    ...
                    tr.innerHTML = `
                        <td style="${styleLigne}">${indicatorSOS}#${sig.id_signalement}</td>
                        ...
                        <td id="inter-col-${sig.id_signalement}" style="${styleLigne}">
                            <button class="btn btn-sm px-3 d-flex align-items-center gap-2" 
                                    style="background-color: #f68d56; color: white; border-radius: 10px; border: none; font-weight: 500; height: 34px;" 
                                    onclick="ouvrirIntervention(${sig.id_signalement})">
                                <i class="bi bi-calendar-plus"></i> Planifier
                            </button>
                        </td>
                        <td style="${styleLigne}">${avisContent}</td>
                        <td style="${styleLigne}">
                            <div class="d-flex gap-2">
                                <button class="btn btn-sm d-flex align-items-center justify-content-center" 
                                        style="background-color: #f68d56; color: white; width: 34px; height: 34px; border-radius: 10px; border: none;" 
                                        onclick="ouvrirModifSignalement(${sig.id_signalement}, '${statut}', '${priorite}')" title="Modifier">
                                    <i class="bi bi-pencil-square"></i>
                                </button>
                                <button class="btn btn-sm d-flex align-items-center justify-content-center" 
                                        style="background-color: #f68d56; color: white; width: 34px; height: 34px; border-radius: 10px; border: none;" 
                                        onclick="supprimerSignalement(${sig.id_signalement})" title="Supprimer">
    ...
    fetch(`../controller/InterventionController.php?action=get&id_signalement=${id_sig}`)
        ...
        .then(data => {
            ...
            if (btnSearch) {
                ...
                btnSearch.style.background = '#f68d56';
...
function ouvrirAvisCitoyen(id) {
    const sig = signalementsCache[id];
    if (!sig) return;
    
    document.getElementById('avis_titre').innerText = sig.titre;
    document.getElementById('avis_note').innerText = '⭐'.repeat(sig.note_citoyen);
    document.getElementById('avis_commentaire').innerText = sig.avis_citoyen || "Aucun commentaire laissé.";
