...
class SignalementController {
    ...
    private function redirectToUserList($msg = null) {
        $url = '../view/liste_signalements_user.html';
        if ($msg) {
    ...
    public function citizenUpdateAction($pdo) {
        $cin = $_SESSION['cin'] ?? null;
        $id = intval($_POST['id_signalement'] ?? 0);
        ...
        if (!$cin || !$id) {
            if ($this->isAjaxRequest()) {
                header('Content-Type: application/json; charset=utf-8');
                echo json_encode(['success' => false, 'error' => 'Session invalide.'], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
                exit();
            }
            $this->redirectToUserList();
        }

        $succes = Signalement::updateByCin($pdo, $id, $cin, $titre, $description);
    ...
    public function citizenDeleteAction($pdo) {
        $cin = $_SESSION['cin'] ?? null;
        $id = intval($_POST['id_signalement'] ?? 0);
        ...
        $succes = Signalement::supprimerByCin($pdo, $id, $cin);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        elseif ($_POST['action'] === 'citizenUpdate') $controller->citizenUpdateAction($pdo);
        elseif ($_POST['action'] === 'citizenDelete') $controller->citizenDeleteAction($pdo);
        elseif ($_POST['action'] === 'saveFeedback') {
            $id = intval($_POST['id_signalement'] ?? 0);
            $note = intval($_POST['note'] ?? 0);
            $commentaire = htmlspecialchars($_POST['commentaire'] ?? '');
            $succes = Signalement::saveFeedback($pdo, $id, $note, $commentaire);
            header('Content-Type: application/json');
