...
function profileUrlFromComment(c) {
  const modulesIndex = window.location.pathname.toLowerCase().indexOf('/modules/');
  const base = modulesIndex !== -1 ? window.location.pathname.slice(0, modulesIndex) : '';
  if (c.auteur_citoyen_id) {
    return `${base}/Modules/Social/View/profile.html?id=${encodeURIComponent(c.auteur_citoyen_id)}`;
  }
  if (c.num_cin) {
    return `${base}/Modules/Social/View/profile.html?cin=${encodeURIComponent(c.num_cin)}`;
  }
  return `${base}/Modules/Social/View/profile.html`;
}
  ...
  fetch('../controller/CommentaireController.php?action=getAll')
    ...
    .then(data => {
      ...
      data.forEach(c => {
        ...
        const profileHref = profileUrlFromComment(c);
        const isCensored = c.statut === 'censure' || c.contenu === '***************';
        const statutBadge = c.statut === 'bloqué'
          ? '<span class="badge bg-danger">Bloqué</span>'
          : isCensored
          ? '<span class="badge bg-warning text-dark">Censuré</span>'
        ...
        tr.innerHTML = `
          ...
          <td class="text-truncate ${isCensored ? 'text-danger fw-bold' : ''}" style="max-width:220px;" title="${(c.contenu || '').replace(/"/g,'&quot;')}">${truncate(c.contenu)}</td>
        ...
        const authorCell = tr.children[1];
        if (authorCell && authorCell.firstChild && authorCell.firstChild.nodeType === Node.TEXT_NODE) {
          const link = document.createElement('a');
          link.href = profileHref;
          link.className = 'text-decoration-none';
          link.textContent = authorCell.firstChild.textContent;
          authorCell.replaceChild(link, authorCell.firstChild);
        }
...
function sauvegarderEdit() {
  ...
  fetch('../controller/CommentaireController.php', { method: 'POST', body: form })
    ...
    .catch((error) => {
      showFlash(error.message === 'admin_required'
        ? 'Seuls les administrateurs peuvent modifier les commentaires.'
        : 'Erreur réseau.', 'danger');
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('edit_contenu').addEventListener('keydown', function (event) {
