document.addEventListener('DOMContentLoaded', function () {
    ...
    if (params.get('error') === 'validation') {
        const box = document.getElementById('alertBox');
        if (box) box.innerHTML = '<div class="alert alert-danger">Veuillez corriger les champs invalides avant d\'envoyer la demande.</div>';
    }
    if (params.get('error') === 'image') {
        const box = document.getElementById('alertBox');
        if (box) box.innerHTML = '<div class="alert alert-danger">Veuillez ajouter une image locale valide (PNG, JPG, WEBP ou GIF, max 5 MB).</div>';
    }
    ...
    form.addEventListener('submit', function (event) {
        ...
        const imageFile   = document.getElementById('image_projet').files[0];
        ...
        if (!imageFile || !/^image\/(png|jpeg|webp|gif)$/.test(imageFile.type) || imageFile.size > 5 * 1024 * 1024) {
            alert('Veuillez ajouter une image locale valide (PNG, JPG, WEBP ou GIF, max 5 MB).');
            document.getElementById('image_projet').focus();
            event.preventDefault(); return;
        }
